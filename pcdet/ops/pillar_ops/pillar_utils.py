...
def bev_spatial_shape(point_cloud_range, pillar_size):
    H = round((point_cloud_range[3] - point_cloud_range[0]) / pillar_size)
    W = round((point_cloud_range[4] - point_cloud_range[1]) / pillar_size)
    return int(W), int(H)
...
class PillarQueryAndGroup(nn.Module):
    ...
    def forward(self, xyz, xyz_batch_cnt, point_features):
        ...
        # MAFF-style f_cluster: point offset to pillar mean (computed per pillar)
        pillar_set_indices_long = pillar_set_indices.long()
        num_pillars = pillars.shape[0]
        sum_xyz = torch.zeros((num_pillars, 3), device=xyz.device, dtype=xyz.dtype)
        cnt_xyz = torch.zeros((num_pillars, 1), device=xyz.device, dtype=xyz.dtype)
        ones = torch.ones((group_point_xyz.shape[0], 1), device=xyz.device, dtype=xyz.dtype)
        sum_xyz.index_add_(0, pillar_set_indices_long, group_point_xyz)
        cnt_xyz.index_add_(0, pillar_set_indices_long, ones)
        mean_xyz = sum_xyz / torch.clamp(cnt_xyz, min=1.0)
        group_pillar_means = mean_xyz[pillar_set_indices_long]
        group_pillar_clusters = group_point_xyz - group_pillar_means

        group_features = torch.cat([
            group_point_features.detach(),
            group_point_xyz.detach(),
            group_pillar_centers.detach(),
            group_pillar_clusters.detach()
        ], dim=1)
