...
from pcdet.utils import box_utils, calibration_kitti, common_utils, object3d_kitti, vod_utils
from pcdet.datasets import DatasetTemplate

def calc_dist(x, y):
    l_x, l_y = x.shape[0], y.shape[0]
    x = x[:, :3].reshape([l_x, 3, 1])
    y = y[:, :3].T.reshape([1, 3, l_y])
    d = x - y
    d = np.power(d, 2)
    return d.sum(1)
...
class VODDataset(DatasetTemplate):
    ...
    def get_lidar(self, idx):
        ...
        # filtered_points = points[:, self.used_feature_index]
    ...
    def get_image_shape(self, idx):
        ...
        if not img_file.exists():
            ...
            print(f"[DEBUG] root_split_path: {self.root_split_path}")
            print(f"[DEBUG] split: {self.split}")
            raise FileNotFoundError(f"Missing image file: {img_file}")
        return np.array(io.imread(img_file).shape[:2], dtype=np.int32)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        current_class = [0, 1, 2]
        results = {}
        results.update(vod_utils.get_official_eval_result(eval_gt_annos, eval_det_annos, current_class, custom_method=0))
        results.update(vod_utils.get_official_eval_result(eval_gt_annos, eval_det_annos, current_class, custom_method=3))
        ...
        if "points" in get_item_list:
            if self.use_virtual_points:
                ...
                if len(gt_real_points) == 0:
                    points = np.ones([real_points.shape[0], virtual_points.shape[1] + 2])
                    points[:, :7] = real_points
                else:
                    if self.no_dup:
                        dist = calc_dist(gt_real_points, real_points)
                        flag = np.abs(dist.min(0)) > 0.001
                        real_points = real_points[flag]
                    points = np.ones([virtual_points.shape[0] + real_points.shape[0] + gt_real_points.shape[0], virtual_points.shape[1] + 2])
            ...
            # if self.use_radar_points:
            #     points = self.transform_radar2lidar(points, sample_idx)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        create_kitti_infos(
            ...
            data_path=ROOT_DIR / 'data' / 'view_of_delft_PUBLIC' / 'vod_radar_5frames',
            save_path=ROOT_DIR / 'data' / 'view_of_delft_PUBLIC' / 'vod_radar_5frames'
        )
