...
class RadarOccupancy2DV2(nn.Module):
    def __init__(self, model_cfg, point_cloud_range, voxel_size, occupancy_init=0.01, radar_backbone='pillarnet',
                 use_mask=False) -> None:
        super().__init__()
        self.model_cfg = model_cfg
    ...
    def forward(self, batch_dict):
        ...
        # =======================
        # Write occ prob for later fusion/attn (hard-coded key)
        batch_dict["radar_occ_prob"] = radar_occupancy.detach()

        # =======================

        # =======================
        # Debug (rank0 + low frequency)
        def _is_rank0():
            return (not torch.distributed.is_available()) or (not torch.distributed.is_initialized()) or (
                        torch.distributed.get_rank() == 0)

        if not hasattr(self, "_dbg_cnt"):
            self._dbg_cnt = 0
        self._dbg_cnt += 1
        DBG_EVERY = 200  # 写死：每 200 step 打印一次

        if _is_rank0() and ((not self.training) or (self._dbg_cnt % DBG_EVERY == 0)):
            p = batch_dict["radar_occ_prob"]  # [B,1,H,W]
            flat = p.flatten(1)
            q50 = torch.quantile(flat, 0.50, dim=1).mean().item()
            q90 = torch.quantile(flat, 0.90, dim=1).mean().item()
            q99 = torch.quantile(flat, 0.99, dim=1).mean().item()
            area_02 = (p > 0.2).float().mean().item()
            area_03 = (p > 0.3).float().mean().item()
            # print(f"[RadarOcc] shape={tuple(p.shape)} mean={p.mean().item():.4f} "
            #     f"q50={q50:.4f} q90={q90:.4f} q99={q99:.4f} "
            #     f"area(>0.2)={area_02:.4f} area(>0.3)={area_03:.4f}")
        # =======================
        ...
        if self.use_mask:
            ...
            batch_dict["spatial_features_occupy"] = image_bev_features * (
                        radar_occupancy * radar_ratio + (1 - radar_ratio))
        else:
            if self.fuse_mode == 'MUL':
                feats = image_bev_features * radar_occupancy
            elif self.fuse_mode == 'SE':
                feats = self.fuse_squeeze(
                    self.fuse(torch.concat([image_bev_features * radar_occupancy, image_bev_features], dim=1)))
