...
class DCNv3(nn.Module):
    def __init__(
        ...
        # if Dv3 is None:
        #     raise RuntimeError(
        #         "DCNv3 is not installed. Please set use_ffn=False in MDFENNeck or install DCNv3."
        #     )
        self.dw_conv = nn.Sequential(
            nn.Conv2d(
                channels,
        ...
        self.mask = nn.Linear(
            channels,
            group * kernel_size * kernel_size)
        self.input_proj = nn.Linear(channels, channels)   # in_channels, inner_channels (== out_channels)
        self.output_proj = nn.Linear(channels, channels)  # inner_channels, out_channels
        self._reset_parameters()
        
        if center_feature_scale:   # center_feature_scale = False
            self.center_feature_scale_proj_weight = nn.Parameter(
                torch.zeros((group, channels), dtype=torch.float))
            self.center_feature_scale_proj_bias = nn.Parameter(
                torch.tensor(0.0, dtype=torch.float).view((1,)).repeat(group, ))
...
class DeformFFN(nn.Module):
    ...
    def __init__(self,
                 ...
                 groups,           # customize
                 mlp_ratio=4.,
                 drop=0.,       # the ratio for Dropout in MLP 
                 act_layer='GELU',
                 norm_layer='LN',
                 layer_scale=1.0,
                 offset_scale=1.0, # customize
        ...
        if core_op == 'DCNv3':
            ...
            raise ValueError(f'core_op should select one type of DCNv3 operators.')
    ...
    def forward(self, x):
        x = x + self.gamma1 * self.norm1(self.dcn(x))
        x = x + self.gamma2 * self.norm2(self.mlp(x))
        return x