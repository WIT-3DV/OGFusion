...
from pcdet.models.fusion.mambev.layers import Roll, Ref2Int, Traverse2D, batched
from pcdet.models.fusion.mambev.utils.sequence_utils import (
    DeviceLikeType,
    get_merge_mask_and_sort,
)

# NOTE: torch._dynamo is only available in PyTorch 2.0+, commented out for 1.x compatibility
...
class BaseHydraBlock(BaseModule, metaclass=ABCMeta):
    def __init__(
        ...
        **kwargs,
        ...
        if average_method == "slots":
            self.post_average = average_slots
        elif average_method == "cams":
            self.post_average = average_cams
        elif average_method is None:
