...
def ref2int_factory(convert_method: ConvertMethodType, offset: int = 0):
    if convert_method == "floor":
        convert_func = torch.floor
    elif convert_method == "ceil":
        convert_func = torch.ceil
    elif convert_method == "round":
...
def _parse_sequential(sequential: nn.Sequential) -> Tuple[int, ...]:
    ...
    for m in sequential:
        ...
        elif class_name == "Rearrange":
            pass
        elif class_name == "ContiguousRearrange":
            ops.append(4)
        elif class_name == "SkipFlip":
            ops.append(5)
        else:
            raise ValueError()
    return tuple(ops)
...
class Traverse2D(nn.Module):
    ...
    def forward(
        ...
        H: Optional[Union[int, Number]] = None,
        W: Optional[Union[int, Number]] = None,
