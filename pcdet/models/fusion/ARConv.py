...
class ARConv(nn.Module):
    ...
    def forward(self, x, epoch, hw_range):
        ...
        out = x_offset * m + bias
        # # 打印出 N_X 和 N_Y
        # print(f"Calculated N_X: {N_X}, N_Y: {N_Y}")
        #
        # # 打印偏移量 (p)
        # print(f"Shape of p: {p.shape}")
        #
        # # 打印出 x_offset 的形状和具体内容，检查是否与卷积核的变化相关
        # print(f"x_offset shape: {x_offset.shape}")
        # print(f"x_offset: {x_offset[0, :, 0, 0]}")  # 假设你想查看第一个样本的第一个通道和第一行第一列的值
        #
        # # 输出最终的卷积核
        # print(f"Output (x_offset * m + bias): {out.shape}")
        return out
...
class ARConv2dLike(nn.Module):
    """
    把 ARConv 包装成 Conv2d 风格：forward(x) 不再要求你每次传 epoch/hw_range。
    - epoch：默认用 buffer 存一个值，可在训练循环里 set_epoch()。
    - hw_range：默认 [1, 3]（ARConv 内有特判，会走 scale=1 的稳定分支），
                 也可在前向时按特征图大小自动设置。
    """
    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, padding=1,
                 default_hw_range=(1, 3)):
        super().__init__()
        assert kernel_size in (3, 5, 7), "ARConv 只支持 3/5/7 系列核"
        self.ar = ARConv(inc=in_channels, outc=out_channels,
                         kernel_size=kernel_size, stride=stride, padding=padding)
        self.register_buffer("_epoch_tensor", torch.tensor(0, dtype=torch.int32))
        self._default_hw_range = list(default_hw_range)

    @torch.no_grad()
    def set_epoch(self, epoch: int):
        self._epoch_tensor.fill_(int(epoch))

    @torch.no_grad()
    def set_hw_range(self, h: int, w: int):
        self._default_hw_range = [int(h), int(w)]

    def forward(self, x):
        # 1) 尝试自动给个 hw_range：用 [1, 3] 是最稳（ARConv 里会走 scale=1）
        #    你也可以换成基于特征图大小的估计，比如 [x.size(2)//64, x.size(3)//64]（需>=1）。
        hw_range = self._default_hw_range
        epoch = int(self._epoch_tensor.item())
        return self.ar(x, epoch=epoch, hw_range=hw_range)